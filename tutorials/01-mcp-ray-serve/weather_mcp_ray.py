...
async def get_forecast(latitude: float, longitude: float) -> str:
    """Fetch a 5-period weather forecast for the given coordinates."""
...
async def lifespan(fastapi_app: FastAPI):
    # Mount MCP Streamable HTTP at /mcp so /health stays at the root.
    # stateless_http=True means no session affinity — each Ray Serve replica
    # handles requests independently, enabling horizontal autoscaling.
...
# Ray Serve entry point: serve run weather_mcp_ray:app
app = WeatherMCP.bind()
